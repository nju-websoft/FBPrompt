...
def evaluate_drop(
    ...
    for query_id in annotations.keys():
        # query_id = qa_pair["id"]
        qa_pair = annotations[query_id]
        max_em_score = 0.0
        max_f1_score = 0.0
        if query_id in predicted_answers:
            predicted = predicted_answers[query_id]
            candidate_answers = [qa_pair["answer_ori"]]
...
if __name__ == "__main__":
    prediction_path = './datasets/drop/valid_pred.json'
    # gold_path = './datasets/drop/drop_dataset_dev.json'
    gold_path = './datasets/drop/inition/valid.json'
    annotations = json.load(open(gold_path, encoding="utf-8"))
    predicted_answers = json.load(open(prediction_path, encoding="utf-8"))
    result = evaluate_drop(annotations, predicted_answers)
    print(result)
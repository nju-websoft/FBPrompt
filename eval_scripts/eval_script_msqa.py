...
def evaluate_msqa(preds, golds, brief=True):
    assert len(preds) == len(golds)
    assert preds.keys() == golds.keys()
    # Normalize the answer
    for k, v in golds.items():
    ...
    if brief:
        result = {
                  'em_f1':  round(100 * em_f, 2),
                  'overlap_f1':  round(100 * overlap_f, 2)}
        return result
    else:
        result = {'em_precision':  round(100 * em_p, 2),
                  'em_recall':  round(100 * em_r, 2),
                  'em_f1':  round(100 * em_f, 2),
                  'overlap_precision':  round(100 * overlap_p, 2),
                  'overlap_recall':  round(100 *overlap_r, 2),
                  'overlap_f1':  round(100 * overlap_f, 2)}
        return result

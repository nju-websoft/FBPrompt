...
from eval_scripts.eval_script_drop import get_metrics
...
def evaluate_quoref(
    ...
    for k, v in predicted_answers.items():
        if isinstance(v[0], list) or isinstance(v[0], tuple):
            # v = [item[0] for item in v]
            predicted_answers[k] = v
...
if __name__ == "__main__":
    # path = './results/quoref/use_correct_incorrect_miss/results_test.json'
    path = './results/quoref/use/results_test.json'
    dataset = read_dataset(path)
    preds, golds = {}, {}
    for sample in dataset:
        if 'gpt_pred' in sample:
            preds[sample['id']] = sample['gpt_pred']
            golds[sample['id']] = sample['answers']
    global_em, global_f1 = evaluate_quoref(golds, preds)
